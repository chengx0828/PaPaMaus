...
def autoregress_logit(df: pd.DataFrame) -> float:
    ...
    """
    df['Delta_zt'] = df['Adj Close'].diff()
    df['Delta_zt-1'] = df['Delta_zt'].shift(1)
    df = df.dropna()
    df['Dependent'] = (df['Delta_zt'] > 0).astype(int)
    X = sm.add_constant(df['Delta_zt-1'])
    y = df['Dependent']
    model = sm.Logit(y, X)
    result = model.fit()
    t_value = result.tvalues['const']
